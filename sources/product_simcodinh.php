<?php  
	...
	{ 
		/* Lấy sản phẩm detail */
		$row_detail = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, motangan$lang, noidung$lang, masp, luotxem, id_brand, id_mau, id_size,id_doday, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giakm, giamoi, gia from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		/* Lấy cấp 1 */
		$pro_list = $d->rawQueryOne("select * from #_product_list where id = ? and type = ? and hienthi > 0 limit 0,1",array($row_detail['id_list'],$type));
		...
		$sql = "select a.ten$lang, a.tenkhongdau$lang, a.photo,a.gia, a.giamoi from #_product a where $where order by a.stt,a.id desc $limit";
	...
	else if($idc!='')
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select a.ten$lang, a.tenkhongdauvi, a.photo,a.gia, a.giamoi from #_product a where $where order by a.stt,a.id desc $limit";
		$product = $d->rawQuery($sql,$params);
		$sqlNum = "select count(*) as 'num' from #_product a where $where order by a.stt,a.id desc";
		$count = $d->rawQueryOne($sqlNum,$params);
		$total = $count['num'];
	...
	else
	{
		...
		/* Lấy tất cả sản phẩm */
		$where = "";
		$where = "a.type = ? and a.hienthi > 0 and a.id_list = '119'";
		$params = array($type);
		...
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product a where $where order by a.stt,a.id desc $limit";
		$sql_tong = "select * from #_product a where $where order by a.stt,a.id desc";
		$product = $d->rawQuery($sql,$params);
		$product_tong = $d->rawQuery($sql_tong,$params);
		$sqlNum = "select count(*) as 'num' from #_product a where $where order by a.stt,a.id desc";
		$count = $d->rawQueryOne($sqlNum,$params);
		$total = $count['num'];
		$url = $func->getCurrentPageURL();
