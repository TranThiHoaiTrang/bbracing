<?php  
	...
	if($type == 'faqs'){
		$static_faqs = $d->rawQuery("select * from #_news where type = ? and hienthi > 0 order by stt,id desc",array($type));
	}
	
	$static = $d->rawQueryOne("select * from #_news where type = ? limit 0,1",array($type));
	$static2 = $d->rawQuery("select * from #_news where type = ? limit 2,100",array($type));
