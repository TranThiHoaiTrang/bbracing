<?php  
	...
	@$iddl = htmlspecialchars($_GET['iddl']);
	@$idnd = htmlspecialchars($_GET['idnd']);
// var_dump($iddl);
	if($id!='')
	{ 
		/* Lấy sản phẩm detail */
	...
	{
		/* Lấy brand detail */
		$pro_doday = $d->rawQueryOne("select * from #_product_doday where id = ? and type = ? limit 0,1",array($idd,$type));
		$noidung_page = $pro_doday['noidung'.$lang];
		/* SEO */
		$title_cat = $pro_doday['ten'.$lang];
		$seoDB = $seo->getSeoDB($pro_doday['id'],'product','man_doday',$pro_doday['type']);
		$seo->setSeo('h1',$pro_doday['ten'.$lang]);
		if(!empty($seoDB['title'.$seolang])) $seo->setSeo('title',$seoDB['title'.$seolang]);
		else $seo->setSeo('title',$pro_doday['ten'.$lang]);
		if(!empty($seoDB['keywords'.$seolang])) $seo->setSeo('keywords',$seoDB['keywords'.$seolang]);
		if(!empty($seoDB['description'.$seolang])) $seo->setSeo('description',$seoDB['description'.$seolang]);
		$seo->setSeo('url',$func->getPageURL());
		$img_json_bar = (isset($pro_doday['options']) && $pro_doday['options'] != '') ? json_decode($pro_doday['options'],true) : null;
		if($img_json_bar == null || ($img_json_bar['p'] != $pro_doday['photo']))
		{
			$img_json_bar = $func->getImgSize($pro_doday['photo'],UPLOAD_PRODUCT_L.$pro_doday['photo']);
			$seo->updateSeoDB(json_encode($img_json_bar),'product_doday',$pro_doday['id']);
		}
		if(count($img_json_bar) > 0)
		{
			$seo->setSeo('photo',$config_base.THUMBS.'/'.$img_json_bar['w'].'x'.$img_json_bar['h'].'x2/'.UPLOAD_PRODUCT_L.$pro_doday['photo']);
			$seo->setSeo('photo:width',$img_json_bar['w']);
			$seo->setSeo('photo:height',$img_json_bar['h']);
			$seo->setSeo('photo:type',$img_json_bar['m']);
		}
		...
		/* breadCrumbs */
		$breadcr->setBreadCrumbs($pro_doday[$sluglang],$title_cat);
		$breadcrumbs = $breadcr->getBreadCrumbs();
	}
	else if($iddl!='')
	{
		/* Lấy brand detail */
		$pro_doday = $d->rawQueryOne("select * from #_product_doday_list where id = ? and type = ? limit 0,1",array($iddl,$type));
		$noidung_page = $pro_doday['noidung'.$lang];
		// var_dump($pro_doday);
		/* SEO */
		$title_cat = $pro_doday['ten'.$lang];
		...
		/* Lấy sản phẩm */
		$where = "";
		// $where = "id_doday_list = ? and type = ? and hienthi > 0";
		// $params = array($pro_doday['id'],$type);
		$where = "id_doday_list_onmybike REGEXP ('".$pro_doday['id']."') and type = ? and hienthi > 0";
		$params = array($type);
	...
	{
		/* Lấy brand detail */
		$pro_doday = $d->rawQueryOne("select * from #_product_nhomdanhmuc where id = ? and type = ? limit 0,1",array($idnd,$type));
		$noidung_page = $pro_doday['noidung'.$lang];
		// var_dump($pro_doday);
		/* SEO */
		$title_cat = $pro_doday['ten'.$lang];
		...
		/* Lấy sản phẩm */
		$where = "";
		$id_list = $d->rawQuery("select id from #_product_list where id_nhomdanhmuc = ? and type = ?",array($idnd,$type));
		$ds_id_list = '';
		foreach($id_list as $v){
			$ds_id_list .= $v['id']. '|';
		}
		$ds_id_list = substr($ds_id_list, 0, -1);
		$where = "id_list REGEXP ? and type = ? and hienthi > 0";
		$params = array($ds_id_list,$type);
