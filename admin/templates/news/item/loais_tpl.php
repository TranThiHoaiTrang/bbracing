...
foreach($quyen_all as $a){
    if($a['quyen'] === 'news_add__item'.$type.''){ 
        $none_add = "";
        // break;
    }
    elseif($a['quyen'] === 'news_edit__item'.$type.''){ 
        $none_edit = "";
        // break;
    }
    elseif($a['quyen'] === 'news_delete__item'.$type.'' ){
        $none_delete = "";
        // break;
    }
}
...
<section class="content">
    ...
    <div class="card-footer text-sm">
    	<a class="btn btn-sm bg-gradient-primary text-white <?=$none_edit?>" href="<?=$linkAdd?>" title="Thêm mới"><i class="fas fa-plus mr-2"></i>Thêm mới</a>
        <a class="btn btn-sm bg-gradient-danger text-white <?=$none_delete?>" id="delete-all" data-url="<?=$linkDelete?><?=$strUrl?>" title="Xóa tất cả"><i class="far fa-trash-alt mr-2"></i>Xóa tất cả</a>
    </div>
</section>