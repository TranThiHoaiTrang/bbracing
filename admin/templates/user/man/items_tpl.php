...
?>
<style>
    .card-header-member::after{content: none;}
</style>
<!-- Content Header -->
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header card-header-member d-flex align-items-center justify-content-between">
            <h3 class="card-title">Danh sách tài khoản khách</h3>
            <?php 
            $linkExcel = "index.php?com=excelAllMember";
            ?>
            <a class="btn btn-sm bg-gradient-success btn-export-excel btn-sm d-inline-block align-middle ml-2 text-white" onclick="actionOrder('<?=$linkExcel?>')" title="Xuất file excel đăng kí nhận tin"><i class="far fa-file-excel mr-1"></i>Xuất file excel đăng kí nhận tin</a>
        </div>
                    ...
                    <tbody>
                        <?php for ($i = 0; $i < count($items); $i++) { ?>
                            <tr>
                                ...
                                <?php 
						            $item_vip = $d->rawQueryOne("select * from #_member_vip where id = '".$items[$i]['id_vip']."'");
						        ?>
                                <td class="align-middle">
                                    <a class="text-dark" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>" title="<?=$item_vip['ten'.$lang]?>"><?=$item_vip['ten'.$lang]?></a>
                                </td>
