<?php 
	...
	switch($act)
	{
		...
		case "delete":
			...
			/* Info vip */
		case "man_vip":
			/* Kiểm tra active user vip */
			if(!isset($config['user']['vip']) || $config['user']['vip'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
			if($func->check_permission())
			{
				$func->transfer("Bạn không có quyền vào trang này", "index.php", false);
				exit;
		...
		case "add_vip":
			...
			$template = "user/man_vip/item_add";
			break;
		case "edit_vip":
			/* Kiểm tra active user vip */
			if(!isset($config['user']['vip']) || $config['user']['vip'] == false) $func->transfer("Trang không tồn tại", "index.php", false);
	...
	function save_item()
	{
		...
		if($data)
		{
			...
			$data['ngaysinh'] = strtotime(str_replace("/","-",$data['ngaysinh']));
			$data['ngayketthuc'] = strtotime(str_replace("/","-",$data['ngayketthuc']));
			$data['hienthi'] = (isset($data['hienthi'])) ? 1 : 0;
		}
	...
	function get_items_vip()
	{
		...
		{
			$keyword = htmlspecialchars($_REQUEST['keyword']);
			$where .= " and (ten LIKE '%$keyword%' or ten LIKE '%$keyword%')";
		}

		$per_page = 10;
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_member_vip where id <> 0 $where order by stt,id desc $limit";
		$items = $d->rawQuery($sql);
		$sqlNum = "select count(*) as 'num' from #_member_vip where id <> 0 $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum);
		$total = $count['num'];
		$url = "index.php?com=user&act=man_vip";
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}

	/* Edit vip */
	function get_item_vip()
	{
		global $d, $func, $curPage, $item;

		$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_vip&p=".$curPage, false);
		
		$item = $d->rawQueryOne("select * from #_member_vip where id = ? limit 0,1",array($id));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=user&act=man_vip&p=".$curPage, false);
	}

	/* Save vip */
	function save_item_vip()
	{
		global $d, $func, $curPage;
		
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=user&act=man_vip&p=".$curPage, false);
		...
		$data['congno'] = (isset($data['congno']) && $data['congno'] != '') ? str_replace(",","",$data['congno']) : 0;

		$discount = $_POST['discount'];
		$idbrand = $_POST['idbrand'];
		$idvip = $_POST['idvip'];
		$idmembervip = $_POST['idmembervip'];
		// var_dump($idmembervip);
		for ($i=0; $i < count($discount) ; $i++) { 
			$data_mbvip['hienthi'] = 1;
			$data_mbvip['discount'] = $discount[$i];
			$data_mbvip['id_brand'] = $idbrand[$i];
			$data_mbvip['id_vip'] = $idvip[$i];
			if($idmembervip[$i] != 0){
				// var_dump($idmembervip[$i]);
				$d->where('id', $idmembervip[$i]);
				$d->update('member_vip_discount',$data_mbvip);
			}
			else{
				$d->insert('member_vip_discount',$data_mbvip);
			}
		}
		// die();

		/* Kiểm tra ten */
		$username = isset($data['ten']) ? $data['ten'] : '';
		$check_username = $d->rawQueryOne("select id from #_member_vip where ten = ? and id <> ? limit 0,1",array($username, $id));
		if(!empty($check_username)) $func->transfer("Tên vip này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man_vip&p=".$curPage, false);

		// var_dump($data);die();
		if($id)
		{
			$d->where('id', $id);
			if($d->update('member_vip',$data)) $func->transfer("Cập nhật dữ liệu thành công", "index.php?com=user&act=man_vip&p=".$curPage);
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=user&act=man_vip&p=".$curPage, false);
		}
		else
		{		
			if($d->insert('member_vip',$data)) $func->transfer("Lưu dữ liệu thành công", "index.php?com=user&act=man_vip&p=".$curPage);
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=user&act=man_vip&p=".$curPage, false);
		}
	}

	/* Delete vip */
	function delete_item_vip()
	{
		global $d, $func, $curPage;
		...
		if($id)
		{
			$d->rawQuery("delete from #_member_vip where id = ?",array($id));
			$func->transfer("Xóa dữ liệu thành công", "index.php?com=user&act=man_vip&p=".$curPage);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		$func->transfer("Không nhận được dữ liệu","index.php?com=user&act=man_vip&p=".$curPage, false);
	}
