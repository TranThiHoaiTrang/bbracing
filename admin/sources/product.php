<?php
	...
	$strUrl = "";
	$arrUrl = array('id_list','id_cat','id_item','id_sub','id_brand','id_doday','id_doday_list','id_doday_onmybike');
	if(isset($_POST['data']))
	{
	...
	switch($act)
	{
		...
		/* độ dày */
		case "man_nhomdanhmuc":
			get_items_nhomdanhmuc();
			$template = "product/nhomdanhmuc/items";
			break;
		case "add_nhomdanhmuc":
			$template = "product/nhomdanhmuc/item_add";
			break;
		case "edit_nhomdanhmuc":
			get_item_nhomdanhmuc();
			$template = "product/nhomdanhmuc/item_add";
			break;
		case "save_nhomdanhmuc":
			save_item_nhomdanhmuc();
			break;
		case "delete_nhomdanhmuc":
			delete_item_nhomdanhmuc();
			break;
		...
		case "man_doday_list":
			get_dodays_list();
			$template = "product/doday_list/items";
			break;
		case "add_doday_list":
			$template = "product/doday_list/item_add";
			break;
		case "edit_doday_list":
			get_doday_list();
			$template = "product/doday_list/item_add";
			break;
		case "save_doday_list":
			save_doday_list();
			break;
		case "delete_doday_list":
			delete_doday_list();
			break;
		...
		case "man_option":
			get_items_option();
			$template = "product/option/items";
			break;
		case "add_option":
			$template = "product/option/item_add";
			break;
		case "edit_option":
			get_item_option();
			$template = "product/option/item_add";
		...
		/* độ dày */
		case "man_option_list":
			get_options_list();
			$template = "product/option_list/items";
			break;
		case "add_option_list":
			$template = "product/option_list/item_add";
			break;
		case "edit_option_list":
			get_option_list();
			$template = "product/option_list/item_add";
			break;
		case "save_option_list":
			save_option_list();
			break;
	...
	function update(){
		...
		foreach ($splistmenu as $key => $value) {
			...
			$data['ungdungvi'] = htmlspecialchars($value['ungdung']);
			$data['luuyvi'] = htmlspecialchars($value['luuy']);
			$data['cauhoivi'] = htmlspecialchars($value['cauhoi']);
			$data['tintucvi'] = htmlspecialchars($value['tintuc']);
...
if($id){
			...
			$data['slugen'] = str_replace('-', '', $data['tenkhongdauen']);
			...
			if(isset($_POST['id_doday_onmybike']) && ($_POST['id_doday_onmybike'] != '')) $data['id_doday_onmybike'] = implode("|", $_POST['id_doday_onmybike']);
				else $data['id_doday_onmybike'] = "";

			if(isset($_POST['id_doday_list_onmybike']) && ($_POST['id_doday_list_onmybike'] != '')) $data['id_doday_list_onmybike'] = implode("|", $_POST['id_doday_list_onmybike']);
			else $data['id_doday_list_onmybike'] = "";

			if(isset($_POST['id_option']) && ($_POST['id_option'] != '')) $data['id_option'] = implode("|", $_POST['id_option']);
			else $data['id_option'] = "";

			if(isset($_POST['id_option_list2']) && ($_POST['id_option_list2'] != '')) $data['id_option_list'] = implode("|", $_POST['id_option_list2']);
			else $data['id_option_list'] = "";
			...
			$data['giado'] = (isset($data['giado']) && $data['giado'] != '') ? str_replace(",","",$data['giado']) : 0;
			$data['giadomoi'] = (isset($data['giadomoi']) && $data['giadomoi'] != '') ? str_replace(",","",$data['giadomoi']) : 0;
			...
			if (isset($result) && ($result != '')) $data['danhsach_option'] = $result;
			else $data['danhsach_option'] = "";
		}
		...
		if($id && $act!='save_copy')
		{
			...
			{
				$file_name1 = $func->uploadName($_FILES['file-taptin']["name"]);
				// var_dump($file_name1);
				if($taptin = $func->uploadImage("file-taptin", $config['product'][$type]['file_type'],UPLOAD_FILE,$file_name1))
				{
					// var_dump($taptin);
					$data['taptin'] = $taptin;
		...
		else
		{
			...
			if(isset($_FILES['file-taptin']))
			{
				...
				if($taptin = $func->uploadImage("file-taptin", $config['product']['file_type'],UPLOAD_FILE,$file_name))
				{
					$data['taptin'] = $taptin;
				}
			}
	...
	/* Get size */
	function get_items_nhomdanhmuc()
	{
		global $d, $func, $strUrl , $curPage, $items, $paging, $type;
		...
		// var_dump($startpoint);
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product_nhomdanhmuc where type = ? $where order by stt asc $limit";
		$items = $d->rawQuery($sql,array($type));
		$sqlNum = "select count(*) as 'num' from #_product_nhomdanhmuc where type = ? $where order by stt asc";
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_nhomdanhmuc" . $strUrl . "&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}

	/* Edit nhomdanhmuc */
	function get_item_nhomdanhmuc()
	{
		global $d, $func, $curPage, $item, $type;

		$id = (isset($_GET['id'])) ? htmlspecialchars($_GET['id']) : 0;

		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);

		$item = $d->rawQueryOne("select * from #_product_nhomdanhmuc where id = ? limit 0,1",array($id));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);
	}

	/* Save nhomdanhmuc */
	function save_item_nhomdanhmuc()
	{
		global $d, $func, $curPage, $config, $type;

		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES['file']["name"]);
				if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_nhomdanhmuc'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_product_nhomdanhmuc where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				}
			}
			if(isset($_FILES['file1']))
			{
				...
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_nhomdanhmuc'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
					$row = $d->rawQueryOne("select id, icon from #_product_nhomdanhmuc where id = ? and type = ? limit 0,1",array($id,$type));
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('product_nhomdanhmuc',$data)) $func->redirect("index.php?com=product&act=man_nhomdanhmuc&type=".$type);
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			if($d->insert('product_nhomdanhmuc',$data)) $func->redirect("index.php?com=product&act=man_nhomdanhmuc&type=".$type);
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);
		}
	}

	/* Delete nhomdanhmuc */
	function delete_item_nhomdanhmuc()
	{
		global $d, $func, $curPage, $type;
		...
		if($id)
		{
			$row = $d->rawQueryOne("select id from #_product_nhomdanhmuc where id = ? and type = ? limit 0,1",array($id,$type));

			if($row['id'])
			{
				$d->rawQuery("delete from #_product_nhomdanhmuc where id = ? and type = ?",array($id,$type));
				$func->redirect("index.php?com=product&act=man_nhomdanhmuc&type=".$type);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if($row['id']) $d->rawQuery("delete from #_product_nhomdanhmuc where id = ? and type = ?",array($id,$type));
			}

			$func->redirect("index.php?com=product&act=man_nhomdanhmuc&type=".$type);
		} 
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_nhomdanhmuc&type=".$type, false);
	}
	...
	function save_item_doday()
	{
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES['file']["name"]);
				if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_doday'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_product_doday where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				}
			}
			if(isset($_FILES['file1']))
			{
				$file_name1 = $func->uploadName($_FILES['file1']["name"]);
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_doday'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
					$row = $d->rawQueryOne("select id, icon from #_product_doday where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
	...
	function get_dodays_list()
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product_doday_list where type = ? $where order by stt,id desc $limit";
		$items = $d->rawQuery($sql,array($type));
		$sqlNum = "select count(*) as 'num' from #_product_doday_list where type = ? $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_doday_list$url_key&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}

	/* Edit list */
	function get_doday_list()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);

		$item = $d->rawQueryOne("select * from #_product_doday_list where id = ? and type = ? limit 0,1",array($id,$type));
		
		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);

		/* Lấy hình ảnh con */
		$gallery = $d->rawQuery("select * from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($id,$com,$type,'man_doday_list',$type));
	}

	/* Save list */
	function save_doday_list()
	{
		global $d, $strUrl, $func, $curPage, $config, $com, $type;

		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);
		...
		// var_dump($data);die();
		/* Post seo */
		if(isset($config['product'][$type]['seo_doday_list']) && $config['product'][$type]['seo_doday_list'] == true)
		{
			$dataSeo = (isset($_POST['dataSeo'])) ? $_POST['dataSeo'] : null;
			if($dataSeo)
			{
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES['file']["name"]);
				if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_doday_list'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_product_doday_list where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				}
			}
			if(isset($_FILES['file1']))
			{
				$file_name1 = $func->uploadName($_FILES['file1']["name"]);
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_doday_list'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
					$row = $d->rawQueryOne("select id, icon from #_product_doday_list where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('product_doday_list',$data))
			{
				/* SEO */
				if(isset($config['product'][$type]['seo_doday_list']) && $config['product'][$type]['seo_doday_list'] == true)
				{
					$d->rawQuery("delete from #_seo where idmuc = ? and com = ? and act = ? and type = ?",array($id,$com,'man_doday_list',$type));

					$dataSeo['idmuc'] = $id;
					$dataSeo['com'] = $com;
					$dataSeo['act'] = 'man_doday_list';
					$dataSeo['type'] = $type;
					$d->insert('seo',$dataSeo);
				}

				$func->redirect("index.php?com=product&act=man_doday_list&type=".$type.$strUrl);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			if($d->insert('product_doday_list',$data))
			{
				$id_insert = $d->getLastInsertId();
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);
		}
	}

	/* Delete list */
	function delete_doday_list()
	{
		global $d, $strUrl, $func, $curPage, $com, $type;
		...
		if($id)
		{
			...
			{
				$func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				$d->rawQuery("delete from #_product_doday_list where id = ?",array($id));

				/* Xóa gallery */
				$row = $d->rawQuery("select id, photo, taptin from #_gallery where id_photo = ? and kind = ? and com = ?",array($id,'man_doday_list',$com));
				...
				if(count($row))
				{
					...
					$d->rawQuery("delete from #_gallery where id_photo = ? and kind = ? and com = ?",array($id,'man_doday_list',$com));
				}

				$func->redirect("index.php?com=product&act=man_doday_list&type=".$type.$strUrl);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);
		}
		elseif(isset($_GET['doday_listid']))
		{
			$doday_listid = explode(",",$_GET['doday_listid']);

			for($i=0;$i<count($doday_listid);$i++)
			{
				$id = htmlspecialchars($doday_listid[$i]);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_doday_list&type=".$type.$strUrl, false);
	}

	/* Get size */
	function get_items_option()
	{
		global $d, $func,$strUrl, $curPage, $items, $paging, $type;
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product_option where type = ? $where order by stt,id desc $limit";
		$items = $d->rawQuery($sql,array($type));
		$sqlNum = "select count(*) as 'num' from #_product_option where type = ? $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_option&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}
	...
	function get_item_option()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_option&type=".$type, false);

		$item = $d->rawQueryOne("select * from #_product_option where id = ? limit 0,1",array($id));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_option&type=".$type, false);
	}

	/* Save option */
	function save_item_option()
	{
		global $d, $func, $curPage, $config, $type;

		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_option&type=".$type, false);
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES['file']["name"]);
				if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_option'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_product_option where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				}
			}
			if(isset($_FILES['file1']))
			{
				$file_name1 = $func->uploadName($_FILES['file1']["name"]);
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_option'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
					$row = $d->rawQueryOne("select id, icon from #_product_option where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('product_option',$data)) $func->redirect("index.php?com=product&act=man_option&type=".$type);
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_option&type=".$type, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			if($d->insert('product_option',$data)) $func->redirect("index.php?com=product&act=man_option&type=".$type);
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_option&type=".$type, false);
		}
	}

	/* Delete option */
	function delete_item_option()
	{
		global $d, $func, $curPage, $type;
		...
		if($id)
		{
			$row = $d->rawQueryOne("select id from #_product_option where id = ? and type = ? limit 0,1",array($id,$type));

			if($row['id'])
			{
				$d->rawQuery("delete from #_product_option where id = ? and type = ?",array($id,$type));
				$func->redirect("index.php?com=product&act=man_option&type=".$type);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_option&type=".$type, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
			...
			for($i=0;$i<count($listid);$i++)
			{
				...
				if($row['id']) $d->rawQuery("delete from #_product_option where id = ? and type = ?",array($id,$type));
			}

			$func->redirect("index.php?com=product&act=man_option&type=".$type);
		} 
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_option&type=".$type, false);
	}
	...
	function get_options_list()
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select * from #_product_option_list where type = ? $where order by stt,id desc $limit";
		$items = $d->rawQuery($sql,array($type));
		$sqlNum = "select count(*) as 'num' from #_product_option_list where type = ? $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_option_list$url_key&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}
	...
	function get_option_list()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);

		$item = $d->rawQueryOne("select * from #_product_option_list where id = ? and type = ? limit 0,1",array($id,$type));
		
		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);

		/* Lấy hình ảnh con */
		$gallery = $d->rawQuery("select * from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($id,$com,$type,'man_option_list',$type));
	}
	...
	function save_option_list()
	{
		...
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);
		...
		if($data)
		{
			...
			if(isset($_POST['option_group']) && ($_POST['option_group'] != '')) $data['id_option'] = implode(",", $_POST['option_group']);
			else $data['id_option'] = "";
		}
		...
		// var_dump($data);die();
		/* Post seo */
		if(isset($config['product'][$type]['seo_option_list']) && $config['product'][$type]['seo_option_list'] == true)
		{
			$dataSeo = (isset($_POST['dataSeo'])) ? $_POST['dataSeo'] : null;
			if($dataSeo)
			{
		...
		if($id)
		{
			...
			{
				$file_name = $func->uploadName($_FILES['file']["name"]);
				if($photo = $func->uploadImage("file", $config['product'][$type]['img_type_option_list'], UPLOAD_PRODUCT,$file_name))
				{
					$data['photo'] = $photo;
					$row = $d->rawQueryOne("select id, photo from #_product_option_list where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				}
			}
			if(isset($_FILES['file1']))
			{
				$file_name1 = $func->uploadName($_FILES['file1']["name"]);
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_option_list'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
					$row = $d->rawQueryOne("select id, icon from #_product_option_list where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('product_option_list',$data))
			{
				/* SEO */
				if(isset($config['product'][$type]['seo_option_list']) && $config['product'][$type]['seo_option_list'] == true)
				{
					$d->rawQuery("delete from #_seo where idmuc = ? and com = ? and act = ? and type = ?",array($id,$com,'man_option_list',$type));

					$dataSeo['idmuc'] = $id;
					$dataSeo['com'] = $com;
					$dataSeo['act'] = 'man_option_list';
					$dataSeo['type'] = $type;
					$d->insert('seo',$dataSeo);
				}

				$func->redirect("index.php?com=product&act=man_option_list&type=".$type.$strUrl);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			if($d->insert('product_option_list',$data))
			{
				$id_insert = $d->getLastInsertId();
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);
		}
	}

	/* Delete list */
	function delete_option_list()
	{
		global $d, $strUrl, $func, $curPage, $com, $type;
		...
		if($id)
		{
			...
			{
				$func->delete_file(UPLOAD_PRODUCT.$row['photo']);
				$d->rawQuery("delete from #_product_option_list where id = ?",array($id));

				/* Xóa gallery */
				$row = $d->rawQuery("select id, photo, taptin from #_gallery where id_photo = ? and kind = ? and com = ?",array($id,'man_option_list',$com));
				...
				if(count($row))
				{
					...
					$d->rawQuery("delete from #_gallery where id_photo = ? and kind = ? and com = ?",array($id,'man_option_list',$com));
				}

				$func->redirect("index.php?com=product&act=man_option_list&type=".$type.$strUrl);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);
		}
		elseif(isset($_GET['option_listid']))
		{
			$option_listid = explode(",",$_GET['option_listid']);

			for($i=0;$i<count($option_listid);$i++)
			{
				$id = htmlspecialchars($option_listid[$i]);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_option_list&type=".$type.$strUrl, false);
	}
	...
	function get_lists()
	{
		...
		$count = $d->rawQueryOne($sqlNum,array($type));
		$total = $count['num'];
		$url = "index.php?com=product&act=man_list" . $strUrl . "&type=".$type;
		$paging = $func->pagination($total,$per_page,$curPage,$url);
	}
	...
	function save_list()
	{
		...
		if($data)
		{
			...
			if(isset($_POST['brand_group']) && ($_POST['brand_group'] != '')) $data['id_brand'] = implode(",", $_POST['brand_group']);
			else $data['id_brand'] = "";

			if(isset($_POST['news_group']) && ($_POST['news_group'] != '')) $data['id_news'] = implode(",", $_POST['news_group']);
			else $data['id_news'] = "";

			if(isset($_POST['nhomdanhmuc_group']) && ($_POST['nhomdanhmuc_group'] != '')) $data['id_nhomdanhmuc'] = implode(",", $_POST['nhomdanhmuc_group']);
			else $data['id_nhomdanhmuc'] = "";
		}
		...
		if($id)
		{
			...
			{
				$file_name1 = $func->uploadName($_FILES['file1']["name"]);
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_list'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
	...
	function save_brand()
	{
		...
		if($id)
		{
			...
			{
				$file_name1 = $func->uploadName($_FILES['file1']["name"]);
				if($photo = $func->uploadImage("file1", $config['product'][$type]['img_type_brand'], UPLOAD_PRODUCT,$file_name1))
				{
					$data['icon'] = $photo;
					$row = $d->rawQueryOne("select id, icon from #_product_brand where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['icon']);
				}
			}
