				...
				foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) 
				{
					...
					for($row=2;$row<=$highestRow;++$row)
					{
						...
						if($tenvi!="")
						{
							...
							// $cell = $worksheet->getCellByColumnAndRow(1, $row);
							// $tenkhongdauvi = $cell->getValue();

							$cell = $worksheet->getCellByColumnAndRow(2, $row);
							$tenvi = $cell->getValue();
							$tenvi_ht = htmlspecialchars($cell->getValue());
							...
							// var_dump($tenkhongdauvi);
							$proimport = $d->rawQueryOne("select * from #_product where id = '".$idsp."' limit 0,1");
							// var_dump("select * from #_product where tenkhongdauvi = '".$tenkhongdauvi."' limit 0,1");
							// var_dump($proimport);
							/* Lấy danh mục cấp 1 */
							$tkdcap1 = $func->changeTitle($cap1);
							// var_dump($tkdcap1);
							$idlist = $d->rawQueryOne("select id from #_product_list where tenvi = ? limit 0,1",array(htmlspecialchars($cap1)));
							// var_dump($idlist);
							/* Lấy danh mục cấp 2 */
							$tkdcap2 = $func->changeTitle($cap2);
							// var_dump($tkdcap2);
							$idcat = $d->rawQueryOne("select id from #_product_cat where tenvi = ? limit 0,1",array(htmlspecialchars($cap2)));
							// var_dump($idcat);
							/* Lấy danh mục brand */
							// $tkdbrand = $func->changeTitle($brand);
							$idbrand = $d->rawQueryOne("select id from #_product_brand where tenvi = ? limit 0,1",array(htmlspecialchars($brand)));
							...
							$data['tenen'] = ($tenen != '') ? htmlspecialchars($tenen) : '';
							$data['tenkhongdauvi'] = ($data['tenvi'] != '') ? $func->changeTitle($data['tenvi']) : '';
							$data['id_brand'] = (int)$idbrand['id'];
							$data['gia'] = $gia;
							$data['giamoi'] = $giamoi;
							$data['giado'] = $giado;
							$data['giadomoi'] = $giadomoi;
							$data['soluongkho'] = (isset($soluongkho) && $soluongkho != '') ? (int)$soluongkho : 0;
							$data['cothemua'] = (isset($cothemua) && $cothemua != '') ? (int)$cothemua : 0;
							$data['photo'] = $proimport['photo'];
