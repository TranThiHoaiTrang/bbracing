<script type="text/javascript">
	...
	/* Refresh filer if empty */
	function refreshFilerIfEmpty1() {
		var idmuc = <?= (isset($id) && $id > 0) ? $id : 0 ?>;
		var com = '<?= (isset($com) && $com != '') ? $com : '' ?>';
		var type = '<?= (isset($type) && $type != '') ? $type : '' ?>';
		var colfiler = $(".col-filer").val();
		var actfiler = $(".act-filer").val();
		var val = $(".val-filer").val();
		var cmd = 'refresh';
		$.ajax({
			type: 'POST',
			dataType: 'html',
			url: 'ajax/ajax_filer1.php',
			async: false,
			data: {
				idmuc: idmuc,
				com: com,
			...
			success: function(result) {
				...
				$tmp = '<div class="form-group form-group-gallery">' +
					...
					result +
					'</ul></div></div>';
				$("#filer-gallery1").parents(".form-group").after($tmp);
				createSortFiler();
			}
		});
	}
	...
	$(document).ready(function() {
		...
		$('body').on('change', '.select-braxe-list', function() {
			var id = $(this).val();
			var child = $(this).data("child");
			// var level = parseInt($(this).data('level'));
			var table = $(this).data('table');
			...
			$.ajax({
				...
				success: function(result) {
					var op = "<option value='0'>Chọn Vehicles list</option>";
					// if(level == 0)
					// {
					// 	$("#id_cat").html(op);
					// 	$("#id_item").html(op);
		...
		$('body').on('change', '.select-option-list', function() {
			var id = $(this).val();
			var child = $(this).data("child");
			// var level = parseInt($(this).data('level'));
			var table = $(this).data('table');
			...
			$.ajax({
				...
				success: function(result) {
					var op = "<option value='0'>Chọn option list</option>";
					// if(level == 0)
					// {
					// 	$("#id_cat").html(op);
					// 	$("#id_item").html(op);
					// 	$("#id_sub").html(op);
					// }
					// console.log("#id_option_list2");
					$("#id_option_list2").html(result);
				}
			});
			return false;
		});
		...
		// console.log($("#filer-gallery1").data());
		$("#filer-gallery1").filer({
			...
			captions: {
				button: "Thêm hình",
				feedback: "Vui lòng chọn video",
				feedback2: "Những hình đã được chọn",
				drop: "Kéo hình vào đây để upload",
				removeConfirmation: "Bạn muốn loại bỏ video này ?",
				errors: {
					filesLimit: "Chỉ được upload mỗi lần {{fi-limit}} video",
					filesType: "Chỉ hỗ trợ tập tin là video có định dạng: {{fi-extensions}}",
					filesSize: "Hình {{fi-name}} có kích thước quá lớn. Vui lòng upload video có kích thước tối đa {{fi-maxSize}} MB.",
					filesSizeAll: "Những video bạn chọn có kích thước quá lớn. Vui lòng chọn những video có kích thước tối đa {{fi-maxSize}} MB."
				}
			},
			afterShow: function() {
				var jFilerItems = $(".my-jFiler-items .jFiler-items-list li.jFiler-item");
			...
			uploadFile: {
				url: "ajax/ajax_upload1.php",
				data: $("#filer-gallery1").data(),
				type: 'POST',
				enctype: 'multipart/form-data',
				dataType: 'json',
				async: false,
		...
		$(function() {
			$(".danhsach_option").select2({
				placeholder: 'Nhập 2 hoặc nhiều hơn để tìm',
				minimumInputLength: 2,
				multiple: true,
				ajax: {
					url: "ajax/ajax_itemsearch.php",
					dataType: 'json',
					type: "POST",
					data: function(term) {
						return {
