<?php
	...
	if($cmd == 'refresh' && ($idmuc > 0 || $hash != ''))
	{
		if($idmuc)
		{
			$gallery = $d->rawQuery("select stt, id, photo, tenvi, tenen from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($idmuc,$com,$type,$kind,$type));
		}
		else if($hash != '')
		{
			$gallery = $d->rawQuery("select stt, id, photo, tenvi, tenen from #_gallery where hash = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($hash,$com,$type,$kind,$type));
		}
	...
	else if($cmd == 'info' && $id > 0 && ($idmuc > 0 || $hash != ''))
	{
		...
		if($d->update('gallery',$data))
		{
			...
			if($gallery)
			{
				for($i=0;$i<count($gallery);$i++)
				{
					echo $func->galleryFiler($gallery[$i]['stt'],$gallery[$i]['id'],$gallery[$i]['photo'],$gallery[$i]['tenvi'],$gallery[$i]['tenen'],$com,$colfiler);
				}
