<?php
	...
	if($cmd == 'refresh' && ($idmuc > 0 || $hash != ''))
	{
		if($idmuc)
		{
			$gallery = $d->rawQuery("select stt, id, photo, tenvi from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($idmuc,$com,$type,$kind,$val));
		}
		else if($hash != '')
		{
			$gallery = $d->rawQuery("select stt, id, photo, tenvi from #_gallery where hash = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($hash,$com,$type,$kind,$val));
		}
	...
	else if(($cmd == 'updateNumb' && $idmuc > 0 && $listid) || ($hash != ''))
	{
		if($idmuc)
		{
			$row = $d->rawQuery("select id, stt from #_gallery where id_photo = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($idmuc,$com,$type,$kind,$val));
		}
		else if($hash != '')
		{
			$row = $d->rawQuery("select id, stt from #_gallery where hash = ? and com = ? and type = ? and kind = ? and val = ? order by stt,id desc",array($hash,$com,$type,$kind,$val));
		}
