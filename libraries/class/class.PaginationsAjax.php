<?php
	class PaginationsAjax
	{
		...
		function getAllPageLinks($count, $href, $elShow)
		{
			...
			{
				if($_GET["p"] == 1) 
					$output = $output . '<a class="first disabled"><<</a>';
					// $output = $output . '<a class="first disabled">First</a><a class="prev disabled">Prev</a>';
				else	
					$output = $output . '<a class="prev" onclick="loadPagingAjax(\'' . $href . ($_GET["p"]-1) . '\',\''.$elShow.'\')" ><<</a>';
				...
				if($_GET["p"] < $pages)
					$output = $output . '<a class="last" onclick="loadPagingAjax(\'' . $href . ($pages) . '\',\''.$elShow.'\')" >>></a>';
					...
					$output = $output . '<a class="next disabled">>></a>';
