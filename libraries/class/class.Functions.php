...
class Functions
{
	...
	public function get_payments($id = 0)
	{
		if ($id == 'paypal') {
			...
			return 'Visa';
		} elseif ($id == 'master_card') {
			return 'Master Card';
		} elseif ($id == 'congno') {
			return 'Công nợ';
		} elseif ($id == 'lua_chon_khac') {
	...
	/* Format money */
	public function format_money_erou($price = 0,$unit = '<i class="fas fa-euro-sign"></i>', $html = false)
	{
		$str = '';
		if ($price) {
	...
	public function checkLogin()
	{
		...
		if (isset($_SESSION[$login_member]) || isset($_COOKIE['login_member_id'])) {
			...
			if ($iduser) {
				$row = $this->d->rawQueryOne("select login_session, id, username, dienthoai, diachi, email, ten, id_vip, id_member from #_member where id = ? and hienthi > 0", array($iduser));
				...
				if ($row['id']) {
					...
					if ($login_session == $row['login_session']) {
						...
						$_SESSION[$login_member]['id_vip'] = $row['id_vip'];
						$_SESSION[$login_member]['id_member'] = $row['id_member'];
					} else $flag = false;
				} else $flag = false;
	...
	public function galleryFiler($stt = 1, $id = 0, $photo = '', $name = '', $folder = '', $col = '')
	{
		...
		$str .= '<input type="text" class="form-control form-control-sm my-jFiler-item-info rounded" value="' . $nameen . '" placeholder="Tiêu đề en" data-info="tieudeen" data-id="' . $id . '"/>';
		$str .= '</div>';
		$str .= '</div>';
		$str .= '</li>';
		return $str;
	...
	public function mbstring_binary_safe_encoding(bool $reset = false)
	{
		static $encodings = [];
	...
	/* Change title */
	public function remove_accents($string, $locale = null)
	{
	...
	public function url_title($str, string $separator = '-', bool $lowercase = true): string
	{
		if ($separator === 'dash') {
			$separator = '-';
	...
	function orderPayments()
	{
		...
		if (isset($_REQUEST['httt']) && ('paypal' == (int)$_REQUEST['httt'])) $selected = "selected";
		else $selected = "";
		$str .= '<option value= paypal ' . $selected . '>Paypal</option>';
		if (isset($_REQUEST['httt']) && ('congno' == (int)$_REQUEST['httt'])) $selected = "selected";
		else $selected = "";
		$str .= '<option value= congno ' . $selected . '>Ví momo</option>';
		$str .= '</select>';
	...
	public function get_ajax_category($table = '', $level = '', $type = '', $title_select = 'Chọn danh mục', $class_select = 'select-category')
	{
		...
		if ($level == 'list') {
			...
			$data_level = '';
			$data_table = 'data-table="#_product_doday_list"';
			$class_select = 'select-braxe-list';
			$data_child = 'data-child="id_doday_list"';
		} else if ($level == 'doday_list') {
			$data_level = '';
			$data_type = '';
			$class_select = '';

			$idlist = (isset($_REQUEST['id_doday'])) ? htmlspecialchars($_REQUEST['id_doday']) : 0;
			$where .= ' and id_doday = ?';
			array_push($params, $idlist);
		}
	...
	public function get_ajax_category_brxe_list($id)
	{
		global $d, $type;
		// var_dump($id);
		if ($id) {
			$temps = $d->rawQueryOne("select id_doday,id_doday_list from #_product where id = ? and type = ? limit 0,1", array($id, $type));
			// var_dump($temps);
			$arr_brand = $temps['id_doday'];
			$arr_brand_list = $temps['id_doday_list'];
			$doday_list = $d->rawQueryOne("select tenvi,id from #_product_doday_list where id = ? and type = ? limit 0,1", array($arr_brand_list, $type));
			// $arr_brand = explode(',', $temps['id_doday']);

			// var_dump($arr_brand);
			// for ($i = 0; $i < count($arr_brand); $i++) $temp[$i] = $arr_brand[$i];
			// var_dump("select tenvi,id from #_product_doday_list where id_doday = ? and type = ? limit 0,1", array($arr_brand, $type));
			$temp = $doday_list;
		}
		// var_dump($temp);
		if ($arr_brand) {
			$row_brand = $d->rawQuery("select tenvi, id from #_product_doday_list where type = ? and id_doday = ? order by stt,id desc", array($type, $arr_brand));
		} else {
			$row_brand = $d->rawQuery("select tenvi, id from #_product_doday_list where type = ? order by stt,id desc", array($type));
		}
		$str = '<select id="id_doday_list" name="data[id_doday_list]" class="form-control select2" >';
		$str .= '<option value="0"> Chọn Vehicles list</option>';
		for ($i = 0; $i < count($row_brand); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_brand[$i]['id'], $temp)) $selected = 'selected="selected"';
				else $selected = '';
			} else {
				$selected = '';
			}
	...
	public function get_ajax_category_list($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_brand from #_product_list where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_brand = explode(',', $temps['id_brand']);

			for ($i = 0; $i < count($arr_brand); $i++) $temp[$i] = $arr_brand[$i];
		}

		$row_brand = $d->rawQuery("select tenvi, id from #_product_brand where type = ? order by stt,id desc", array($type));

		$str = '<select id="brand_group" name="brand_group[]" class="form-control select2" multiple>';
		$str .= '<option value="0"> Chọn brand</option>';
		for ($i = 0; $i < count($row_brand); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_brand[$i]['id'], $temp)) $selected = 'selected="selected"';
				else $selected = '';
	...
	public function get_ajax_category_brand_list($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_brand from #_news where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_brand = explode(',', $temps['id_brand']);

			for ($i = 0; $i < count($arr_brand); $i++) $temp[$i] = $arr_brand[$i];
		}

		$row_brand = $d->rawQuery("select tenvi, id from #_product_brand where type = 'san-pham' order by stt,id desc");

		$str = '<select id="brand_group" name="brand_group[]" class="form-control select2" >';
		$str .= '<option value="0"> Chọn brand</option>';
		for ($i = 0; $i < count($row_brand); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_brand[$i]['id'], $temp)) $selected = 'selected="selected"';
	...
	public function get_ajax_category_brandxe($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_doday from #_product_doday_list where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_brand = explode(',', $temps['id_doday']);

			for ($i = 0; $i < count($arr_brand); $i++) $temp[$i] = $arr_brand[$i];
		}

		$row_brand = $d->rawQuery("select tenvi, id from #_product_doday where type = 'san-pham' order by stt,id desc");
	...
	public function get_ajax_category_baohanh_list($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_baohanh from #_news where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_baohanh = explode(',', $temps['id_baohanh']);

			for ($i = 0; $i < count($arr_baohanh); $i++) $temp[$i] = $arr_baohanh[$i];
		}

		$row_baohanh = $d->rawQuery("select tenvi, id from #_news where type = 'dia_chi_nhan_baohanh' order by stt,id desc");

		$str = '<select id="baohanh_group" name="baohanh_group[]" class="form-control select2" >';
		$str .= '<option value="0"> Chọn baohanh</option>';
		for ($i = 0; $i < count($row_baohanh); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_baohanh[$i]['id'], $temp)) $selected = 'selected="selected"';
				else $selected = '';
			} else {
				$selected = '';
			}
			$str .= '<option value="' . $row_baohanh[$i]["id"] . '" ' . $selected . ' > ' . $row_baohanh[$i]["tenvi"] . '</option>';
		}
		$str .= '</select>';
	...
	public function get_ajax_category_option($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_option from #_product_option_list where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_brand = explode(',', $temps['id_option']);

			for ($i = 0; $i < count($arr_brand); $i++) $temp[$i] = $arr_brand[$i];
		}

		$row_brand = $d->rawQuery("select tenvi, id from #_product_option where type = 'san-pham' order by stt,id desc");

		$str = '<select id="option_group" name="option_group[]" class="form-control select2" >';
	...
	public function get_ajax_news_km($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_news from #_product_list where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_news = explode(',', $temps['id_news']);

			for ($i = 0; $i < count($arr_news); $i++) $temp[$i] = $arr_news[$i];
		}

		$row_news = $d->rawQuery("select tenvi, id from #_news where type = ? order by stt,id desc", array('chuong-trinh-giam-gia'));

		$str = '<select id="news_group" name="news_group[]" class="form-control select2" >';
		$str .= '<option value="0"> Chọn chương trình giảm giá</option>';
		for ($i = 0; $i < count($row_news); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_news[$i]['id'], $temp)) $selected = 'selected="selected"';
				else $selected = '';
			} else {
				$selected = '';
			}
			$str .= '<option value="' . $row_news[$i]["id"] . '" ' . $selected . ' > ' . $row_news[$i]["tenvi"] . '</option>';
		}
		$str .= '</select>';

		return $str;
	}
	public function get_ajax_danhmuc($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_nhomdanhmuc from #_product_list where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_news = explode(',', $temps['id_nhomdanhmuc']);

			for ($i = 0; $i < count($arr_news); $i++) $temp[$i] = $arr_news[$i];
		}

		$row_news = $d->rawQuery("select tenvi, id from #_product_nhomdanhmuc where type = 'san-pham' order by stt,id desc");

		$str = '<select id="nhomdanhmuc_group" name="nhomdanhmuc_group[]" class="form-control select2" >';
		$str .= '<option value="0"> Chọn nhóm danh mục</option>';
		for ($i = 0; $i < count($row_news); $i++) {
			if (isset($temp) && count($temp) > 0) {
				if (in_array($row_news[$i]['id'], $temp)) $selected = 'selected="selected"';
				else $selected = '';
	...
	public function get_ajax_category_brand($id)
	{
		global $d, $type;

		if ($id) {
			$temps = $d->rawQueryOne("select id_list from #_product_brand where id = ? and type = ? limit 0,1", array($id, $type));
			$arr_brand = explode(',', $temps['id_list']);

			for ($i = 0; $i < count($arr_brand); $i++) $temp[$i] = $arr_brand[$i];
		}

		$row_brand = $d->rawQuery("select tenvi, id from #_product_list where type = ? order by stt,id desc", array($type));

		$str = '<select id="list_group" name="list_group[]" class="form-control select2" multiple>';
	...
	function chuyendoitigia($v)
	{
		...
		return $func->hienthichuyendoitigia($giakhuyenmai, $v);
	}
	function hienthichuyendoitigia($giakhuyenmai, $v)
	{
		global $d, $func, $lang;
		$str = '';
		$brand_sp = $d->rawQueryOne("select * from #_product_brand where type = ? and id = ? and hienthi > 0 order by stt,id desc limit 1", array('san-pham', $v['id_brand']));
		...
		if ($lang == 'vi') {
			if ($giakhuyenmai) {
				...
				';
				$sale = abs(round(((($giakhuyenmai / $v['gia']) * 100) - 100), 1));
				if ($sale) {
				...
				if ($v['giamoi']) {
					...
					<span class="giadel">' . $func->format_money($v['gia']) . '</span>
					';
					$sale = abs(round(((($v['giamoi'] / $v['gia']) * 100) - 100), 1));
					if ($sale) {
						$str .=  '<span class="batch"><i class="fas fa-long-arrow-alt-down"></i>' . $sale . ' %</span>';
					}
				} else {
				...
				if ($v['gia']) {
					$str .= '<span class="giamoi">' . $func->format_money($v['gia']) . '</span>';
				} else {
					$tigia = $v['giado'] * str_replace(",", "", $brand_sp['tigia_brand']);
					$tigia_new = round($tigia, 2);

					$str .= '<span class="giamoi">' . $func->format_money($tigia_new) . '</span>';
				}
			}
		} else {
			if ($giakhuyenmai) {
				$str .= '<span class="giamoi">' .$func->format_money_erou($giakhuyenmai) . '</span>';
				if ($v['giadomoi']) {
					$str .= '<span class="giadel">' . $func->format_money_erou($v['giadomoi'])  . '</span>';

					$sale = abs(round(((($giakhuyenmai / $v['giadomoi']) * 100) - 100), 1));
					if ($sale) {

						$str .= '<span class="batch"><i class="fas fa-long-arrow-alt-down"></i>' . $sale . ' %</span>';
					}
				} else {
					$str .= '<span class="giadel">' . $func->format_money_erou($v['giado'])  . '</span>';

					$sale = abs(round(((($giakhuyenmai / $v['giado']) * 100) - 100), 1));
					if ($sale) {
						$str .= '<span class="batch"><i class="fas fa-long-arrow-alt-down"></i>' . $sale . ' %</span>';
					}
				}
			} elseif (!empty($v['giadomoi'])) {
				$str .= '
				<span class="giamoi">' . $func->format_money_erou($v['giadomoi']) . '</span>
				<span class="giadel">' . $func->format_money_erou($v['giado'])  . '</span>
				';

				$sale = abs(round(((($v['giadomoi'] / $v['giado']) * 100) - 100), 1));
				if ($sale) {
					$str .= '
				...
				$str .= '
				<span class="giamoi">' . $func->format_money_erou($v['giado']) . '</span>
				';
			}
		}
		return $str;
	...
	function hienthichuyendoitigia_pro_detail($row_detail,$pro_brand)
	{
		...
		if ($lang == 'vi') {
			...
			if ($giakhuyenmai) {
				...
				$str .= '
				<span class="price-new-pro-detail">'.$func->format_money_erou($row_detail['giadomoi']) .'</span>
				<span class="price-old-pro-detail">'.$func->format_money_erou($row_detail['giado']) .'</span>
				';
			} else {
				$str .= '
				<span class="price-new-pro-detail">'.$func->format_money_erou($row_detail['giado']).'</span>
				';
